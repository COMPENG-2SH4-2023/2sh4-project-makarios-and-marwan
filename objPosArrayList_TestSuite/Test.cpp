...
void testCopyConstructor()
{
	...
	objPosArrayList copyList; // copy constructor
	copyList = thisList;

	int expectedSize = 5;
	int actualSize = copyList.getSize();
	...
	// Then, check the tail element removed
	copyList.getTailElement(currentPos);
	actualCheck = tailPos.isPosEqual(&currentPos);
	...
	for (int i = 0; i < actualSize - 1; i++)
	{
		copyList.getElement(currentPos, i);
		actualCheck = bodyPos.isPosEqual(&currentPos);
	...
	{
		thisList.getElement(currentPos, i);
		cout << currentPos.x << " " << currentPos.y << " " << currentPos.getSymbol() << endl;
	}
